    ...
    while mean([expe.( phase ).conditions.done])~=1 % Keep going while there are some conditions to do
        ...
        while true
            ...
            if previousRespAcc
                ...
                % friends = updateFriend(G.Size(1), G.Size(2), friendsID{friendsLoop(mod(countTrials, length(friendsLoop)) + 1)});
                tmpIdx = mod(countCorrectResponses, length(friendsLoop));
                if tmpIdx == 0
                    tmpIdx = length(friendsLoop);
                end
                friends = updateFriend(G.Size(1), G.Size(2), friendsID{friendsLoop(tmpIdx)});
            ...
            if mod(countCorrectResponses, length(bigFish.availableLocArc1)) == 0
                
                % if all 7 friends are in the lower arch put one of the in the
                % second arch, remove all of them from the first one and change
                % friend
                posOn2ndArch = posOn2ndArch + 1;
                % get the friend in the first arch closer to the available
                % pos in the second arch, move it there and delete the
                % others

                friendOn2ndArch{end + 1} = getTrajectory(friendOnArch{posOn2ndArch}, [bigFish.arcAround2(:,bigFish.availableLocArc2(posOn2ndArch))'], ...
                    [0,0], 4, .5, speedSwim);
                play(G, @()removeFriendsOnFirstArc(friendOnArch, posOn2ndArch));
...
%             if (mod(floor(iter/10), 4) == 0)
    ...
    function removeFriendsOnFirstArc(friendOnArch, friend2keep)
        G
        G.stop();
