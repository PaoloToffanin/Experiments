...
onLocations = onLocations(meanPowerLevel>voicedPowerLevel-20);
offLocations = offLocations(meanPowerLevel>voicedPowerLevel-20);
numberOfSegments = length(onLocations);
if numberOfSegments < 1
    disp('number of segments is 0');
    return;
...
vuvInterpolated = (scoreInterpolated(:)>0.4) & (20*log10(source(:))>medianLevel-15);
fixedStructure.f0 = skeltonF0(:);
vuv = interp1(timeBase,vuvInterpolated*1.0,locations,'nearest','extrap'); % EG 2013-04-05: Added *1.0 for compatibility reasons
