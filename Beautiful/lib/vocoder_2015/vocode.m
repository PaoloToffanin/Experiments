...
%Use second order section (SOS) instead because it reduces the clicking
%that occurs at high frequencies (but NOT all clicking is gone)
[ze, po, k] = butter(ceil(min(p.analysis_filters.order([1, end]))), [p.analysis_filters.lower(1), p.analysis_filters.upper(end)]*2/fs);
[b,a] = zp2sos(ze,po,k); %where a,b = sos,g
...
switch p.envelope.method
    case {'low-pass', 'lp', 'low'}
        ...
        for i=1:nCh
            ...
            %Use SOS here as well to reduce clicking in the output.
            [zlo,plo,klo] = butter(p.envelope.order(i), p.envelope.fc(i)*2/fs, 'low');
            [blo,alo] = zp2sos(zlo,plo,klo);
